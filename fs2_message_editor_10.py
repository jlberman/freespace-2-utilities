...
def wrap_xstr(text):
    """Wrap a string back into XSTR format."""
    return f' XSTR("{text}", -1)'
...
class FS2File:
    ...
    def _extract_send_messages(self, text):
        calls = []
        pattern = re.compile(r'\(\s*(send-message(?:-list|-alt)?)\b', re.IGNORECASE)
...
class MainWindow(QMainWindow):
    ...
    def _make_tabs(self):
        ...
        for b in (self.add_sm_btn, self.add_sml_btn, self.del_smc_btn):
            b.setEnabled(False)
            evtbr.addWidget(b)
    ...
    def _populate_events(self):
        ...
        for event in self.fs2.events:
            ...
            for smc in event.send_message_calls:
                ...
                icon = {'send-message':'💬','send-message-list':'📋','send-message-alt':'🔀'}.get(smc.call_type,'?')
                if smc.call_type == 'send-message':
                    lbl = f"{icon} {smc.call_type}  →  {smc.message_name or '?'}  (from: {smc.sender or '?'})"
                elif smc.call_type == 'send-message-list':
                    lbl = f"{icon} {smc.call_type}  [{len(smc.list_entries)} msgs]"
                else:
    ...
    def _add_smc_to_event(self, call_type):
        ...
        for i in range(root.childCount()):
            ...
            if edata and edata[1] is event:
                if ei.childCount() > 0:
                    self.evt_tree.setCurrentItem(ei.child(ei.childCount() - 1))
                break
